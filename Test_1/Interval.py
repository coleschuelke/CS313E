...
class IntegerInterval (object):
    ...
    def overlap(self, other):
        if self == other:
            return True
        elif self.beginning in range(other.beginning + 1, other.end) or self.end in range(other.beginning + 1, other.end):
            return True
        elif other.beginning in range(self.beginning + 1, self.end) or other.end in range(self.beginning + 1, self.end):
            return True
        else:
            return False
    ...
    def intersection(self, other):
        if not self.overlap(other) or not other.overlap(self):
            return 0
        else:
            section = min([self.end, other.end]) - max([self.beginning, other.beginning])
            return section
